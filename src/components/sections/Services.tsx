            ...
            <div className="container mx-auto px-4">
                <div className="text-center mb-16">
                    ...
                    <p className="text-[var(--muted-foreground)] max-w-2xl mx-auto">
                        {lang === 'sk'
                            ? 'Pomôžeme vám prejsť od starého webu k modernému riešeniu, ktoré vám prinesie nových zákazníkov.'
                            : 'We’ll help you move from an old website to a modern solution that brings in new customers.'}
                    </p>
                </div>
                ...
                <BentoGrid>
                    ...
                    <BentoItem colSpan={2}>
                        <GlassCard className="h-full flex flex-col justify-between border-l-4 border-l-[var(--accent)]">
                            <div>
                                <div className="text-[var(--accent)] font-mono text-sm mb-2">01 // {lang === 'sk' ? 'PRVÝ KROK' : 'STEP ONE'}</div>
                            ...
                            <div className="mt-4 pt-4 border-t border-[var(--card-border)] flex justify-between items-center">
                                <span className="text-sm font-bold">
                                    {lang === 'sk' ? 'Prehľadná a rýchla stránka' : 'Clear and fast website'}
                                </span>
                                <span className="text-xs px-2 py-1 rounded bg-[rgba(20,184,166,0.1)] text-[var(--accent)]">
                                    {lang === 'sk' ? 'Všetko vybavíme za vás' : 'We handle everything'}
                                </span>
                            </div>
                        </GlassCard>
                    </BentoItem>
                    ...
                    <BentoItem>
                        <GlassCard className="h-full bg-[rgba(99,102,241,0.05)] flex flex-col justify-center items-center text-center">
                            ...
                            <h4 className="font-bold mb-1">{lang === 'sk' ? 'Bezpečnosť a istota' : 'Safe and Secure'}</h4>
                            <p className="text-xs text-[var(--muted-foreground)]">{lang === 'sk' ? 'Vaše údaje sú chránené' : 'Your data is protected'}</p>
                        </GlassCard>
                    </BentoItem>

                    {/* Tier 2: Automated helper */}
                    <BentoItem>
                        <GlassCard className="h-full flex flex-col justify-between">
                            <div>
                                <div className="text-[var(--primary)] font-mono text-sm mb-2">02 // {lang === 'sk' ? 'VYLEPŠENIE' : 'UPGRADE'}</div>
                            ...
                            <div className="mt-4">
                                <span className="text-xs font-mono text-[var(--primary)]">
                                    {lang === 'sk' ? 'Odbremení vás od práce' : 'Saves you time'}
                                </span>
                            </div>
                        </GlassCard>
                    </BentoItem>
                    ...
                    <BentoItem colSpan={2}>
                        <GlassCard className="h-full flex flex-col md:flex-row gap-6 items-center bg-gradient-to-r from-[rgba(99,102,241,0.1)] to-transparent">
                            <div className="flex-1">
                                <div className="text-[var(--foreground)] font-mono text-sm mb-2">03 // {lang === 'sk' ? 'DOHĽAD' : 'MANAGEMENT'}</div>
                            ...
                            <div className="flex-shrink-0">
                                <button className="btn btn-primary text-sm">
                                    {lang === 'sk' ? 'Chcem vedieť viac' : 'Learn more'}
                                </button>
                            </div>
                        </GlassCard>
                    </BentoItem>
